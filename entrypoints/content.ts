...
import ViewRouterApp from '@/components/ViewRouterApp.vue';
...
export default defineContentScript({
  ...
  async main(ctx) {
    ...
    if (window !== window.top) {
      ...
      if (migrateHashBoostRouteToQuery()) {
        viewRouterLog('migrated legacy #/sdc-style hash to sdc_boost_vr query param', {
          href: location.href,
        });
      }
      ...
      const viewRouterUI = await createShadowRootUi(ctx, {
        ...
        `;
            ...
            viewRouterApp = createApp(ViewRouterApp);
