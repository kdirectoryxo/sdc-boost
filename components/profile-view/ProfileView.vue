...
interface Props {
  /** When false, profile state is reset (same role as the old dialog `visible`). */
  active: boolean;
  userId: number | null;
  /** Dialog overlay (legacy) vs full view-router page. */
  variant?: 'dialog' | 'page';
  stackLevel?: number;
  dialogId?: string;
}
...
const emit = defineEmits<{
  ...
  /** Page variant: account id for header breadcrumb; `null` while loading or in dialog */
  'profile-breadcrumb': [title: string | null];
}>();

const shellClass = computed(() => {
  if (props.variant === 'page') {
    return 'bg-transparent flex flex-col overflow-hidden flex-1 min-h-0 h-full w-full min-w-0';
  }
  return 'bg-[#131517] flex flex-col overflow-hidden border border-white/[0.06] shadow-2xl h-full w-full max-h-full rounded-lg';
});
...
watch(
  () =>
    [
      ...
      props.active,
      props.userId,
      profileData.value?.db_id,
      profileData.value?.account_id,
    ] as const,
  () => {
    if (props.variant !== 'page' || !props.active) {
      emit('profile-breadcrumb', null);
      return;
    }
    const uid = props.userId;
    const data = profileData.value;
    if (!uid || !data || data.db_id !== uid) {
      emit('profile-breadcrumb', null);
      return;
    }
    emit('profile-breadcrumb', data.account_id ?? null);
  },
  { immediate: true }
);
...
// Fetch profile when the surface is active and `userId` is set
watch([() => props.active, () => props.userId], async ([visible, userId], [prevVisible, prevUserId]) => {
  if (visible && userId) {
    ...
    // Reset when inactive (e.g. dialog closed)
...
async function refreshProfileImages(userId: number, dueToError: boolean = false) {
  ...
  refreshPromise = (async () => {
    try {
      ...
      if (props.active && props.userId === userId) {
...
// Inject styles when surface is active, remove when it hides
watch(
  () => props.active,
  (newValue) => {
    if (newValue) {
...
const showChatButton = computed(() => !props.dialogId || props.dialogId === '');
...
<template>
  <div
    v-if="active"
    class="profile-view-root flex min-h-0 w-full min-w-0 flex-col"
    :class="variant === 'page' ? 'flex-1' : ''"
  >
    <div
      :class="shellClass"
      :style="{
        transform:
          variant === 'dialog' && stackLevel > 0
    ...
    >
      ...
      <div class="flex items-center justify-between px-6 py-4 border-b border-white/[0.06] shrink-0">
        <div class="flex items-center gap-4 flex-1 min-w-0">
          <div class="relative w-12 h-12 shrink-0">
            ...
            <div
              v-else
              class="w-12 h-12 rounded-full bg-white/[0.08] flex items-center justify-center"
            >
            ...
            <div
              ...
              class="absolute -top-1 -right-1 w-4 h-4 border-2 border-blue-500 border-t-transparent rounded-full animate-spin bg-[#1a1d21]"
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2">
              ...
              <a
                ...
                @click.stop
                class="p-1 hover:bg-white/[0.08] rounded transition-colors shrink-0"
        ...
        <div class="flex items-center gap-2 shrink-0">
          <button
            v-if="showChatButton && profileData?.db_id"
            @click="handleOpenChat"
            :disabled="isStartingChat"
            class="p-2 hover:bg-white/[0.08] rounded-md transition-colors shrink-0 disabled:opacity-50 disabled:cursor-not-allowed"
          ...
          <button
            type="button"
            @click="handleCloseChrome"
            class="p-2 hover:bg-white/[0.08] rounded-md transition-colors shrink-0"
            :title="variant === 'page' ? 'Back' : 'Close'"
          >
            <Icon
              :icon="variant === 'page' ? 'mdi:arrow-left' : 'mdi:close'"
              width="20"
              height="20"
              class="text-[#999] hover:text-white"
            />
      ...
      <Tabs v-model="activeTab" class="flex min-h-0 min-w-0 flex-1 flex-col gap-0">
        ...
        <div v-else-if="activeTab === 'profile' && profileData" class="p-6">
              ...
              <div class="md:col-span-1 flex flex-col items-center">
                <div class="relative">
                  <img
                    ...
                    class="w-48 h-48 rounded-2xl object-cover shadow-xl border-2 border-white/[0.06]"
                    @error="handleImageError($event, getPhotoUrl(`${profileData.db_id}/${profileData.photo_file_list[0]}`))"
                    @load="handleImageLoad($event, getPhotoUrl(`${profileData.db_id}/${profileData.photo_file_list[0]}`))"
                  />
                  <div
                    v-else
                    class="w-48 h-48 rounded-2xl bg-white/[0.08] flex items-center justify-center shadow-xl border-2 border-white/[0.06]"
                  >
                  ...
                  <div
                    v-if="isRefreshingImages && isRefreshingDueToError"
                    class="absolute top-2 right-2 w-6 h-6 border-2 border-blue-500 border-t-transparent rounded-full animate-spin bg-[#1a1d21]"
                    title="Refreshing images..."
                  ></div>
                  <div 
                    v-if="profileData.online === 1" 
                    class="absolute bottom-2 right-2 w-5 h-5 bg-green-500 rounded-full border-4 border-[#1a1d21]"
                  ></div>
                </div>
                
                <!-- Membership Badge -->
                <div v-if="profileData.membership" class="mt-4 flex items-center gap-2 px-4 py-2 bg-[#111315] rounded-full border border-white/[0.06]">
              ...
              <div class="md:col-span-2 flex flex-col justify-center space-y-4">
                ...
                <div>
                  <h3 class="text-3xl font-bold text-white mb-2 flex items-center gap-3">
                    ...
                    <span v-else class="px-3 py-1 bg-white/[0.08] text-[#999] text-xs rounded-full font-medium">Offline</span>
                  </h3>
                  <div v-if="profileData.g1_age || (profileData.g2_age && isGender2Real)" class="flex items-center gap-2">
                ...
                <div 
                  ...
                  class="flex items-center gap-3 px-4 py-2.5 bg-[#111315] rounded-lg border border-white/[0.06]"
                >
                ...
                <div class="flex flex-wrap gap-4 pt-2">
                  <div v-if="profileData.validations" class="flex items-center gap-2 px-3 py-1.5 bg-[#111315] rounded-lg border border-white/[0.06]">
                  ...
                  <div v-if="profileData.friend_counter" class="flex items-center gap-2 px-3 py-1.5 bg-[#111315] rounded-lg border border-white/[0.06]">
                  ...
                  <div v-if="profileData.likes" class="flex items-center gap-2 px-3 py-1.5 bg-[#111315] rounded-lg border border-white/[0.06]">
                  ...
                  <div v-if="profileData.messenger_count !== undefined && profileData.messenger_count !== null" class="flex items-center gap-2 px-3 py-1.5 bg-[#111315] rounded-lg border border-white/[0.06]">
          ...
          <!-- Note -->
          <div class="mb-6 bg-[#111315] rounded-xl p-6 border border-white/[0.06]">
            <div class="flex items-center justify-between mb-4">
              ...
              <button
                v-if="!isEditingNote"
                @click="startEditNote"
                class="p-1.5 hover:bg-white/[0.08] rounded transition-colors"
                title="Edit note"
              >
            ...
            <div v-else class="space-y-3">
              <textarea
                ...
                class="w-full min-h-[120px] px-3 py-2 bg-[#1a1d21] border border-white/[0.06] rounded-lg text-sm text-white placeholder-[#666] focus:outline-none focus:border-blue-500 resize-y disabled:opacity-50 disabled:cursor-not-allowed"
              ...
              <div class="flex items-center gap-2">
                ...
                <button
                  ...
                  :disabled="isSavingNote"
                  class="px-4 py-2 bg-white/[0.08] hover:bg-white/[0.12] text-white text-sm font-medium rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Cancel
                </button>
              </div>
          ...
          <!-- Details Table -->
          <div class="bg-[#111315] rounded-xl p-6 border border-white/[0.06] w-full">
            <h4 class="text-xl font-semibold text-white mb-4 flex items-center gap-2">
            ...
            <div class="overflow-x-auto w-full">
              <table class="w-full text-sm">
                <thead>
                  <tr class="border-b-2 border-white/[0.06]">
                ...
                <tbody class="text-white">
                  <tr class="border-b border-white/[0.04] hover:bg-[#1a1d21] transition-colors">
                  ...
                  <tr v-if="profileData.hair_color || profileData.hair_length" class="border-b border-white/[0.04] hover:bg-[#1a1d21] transition-colors">
                  ...
                  <tr v-if="profileData.body_hair" class="border-b border-white/[0.04] hover:bg-[#1a1d21] transition-colors">
                  ...
                  <tr v-if="profileData.height" class="border-b border-white/[0.04] hover:bg-[#1a1d21] transition-colors">
                  ...
                  <tr v-if="profileData.weight" class="border-b border-white/[0.04] hover:bg-[#1a1d21] transition-colors">
                  ...
                  <tr v-if="profileData.body_type" class="border-b border-white/[0.04] hover:bg-[#1a1d21] transition-colors">
                  ...
                  <tr v-if="profileData.race" class="border-b border-white/[0.04] hover:bg-[#1a1d21] transition-colors">
                  ...
                  <tr v-if="profileData.smoke" class="border-b border-white/[0.04] hover:bg-[#1a1d21] transition-colors">
                  ...
                  <tr v-if="profileData.piercings" class="border-b border-white/[0.04] hover:bg-[#1a1d21] transition-colors">
                  ...
                  <tr v-if="profileData.tattoos" class="border-b border-white/[0.04] hover:bg-[#1a1d21] transition-colors">
                  ...
                  <tr v-if="profileData.languages" class="border-b border-white/[0.04] hover:bg-[#1a1d21] transition-colors">
                  ...
                  <tr v-if="profileData.look_imp" class="border-b border-white/[0.04] hover:bg-[#1a1d21] transition-colors">
                  ...
                  <tr v-if="profileData.inte_imp" class="border-b border-white/[0.04] hover:bg-[#1a1d21] transition-colors">
                  ...
                  <tr v-if="profileData.sexuality" class="border-b border-white/[0.04] hover:bg-[#1a1d21] transition-colors">
                  ...
                  <tr v-if="profileData.relationship" class="border-b border-white/[0.04] hover:bg-[#1a1d21] transition-colors">
                  ...
                  <tr v-if="profileData.experience" class="hover:bg-[#1a1d21] transition-colors">
        ...
        <div v-else-if="activeTab === 'images' && profileData" class="p-6">
          <div class="bg-[#111315] rounded-xl p-6 border border-white/[0.06]">
            ...
            <div class="space-y-6">
              ...
              <div class="grid grid-cols-3 gap-4">
                <div
                  ...
                  :key="index"
                  class="aspect-square rounded-lg overflow-hidden bg-[#111315]"
                >
                  <img
            ...
            <div v-if="profileData.video_counter && profileData.video_counter > 0">
              ...
              <div v-if="profileData.video_list && profileData.video_list.length > 0" class="grid grid-cols-3 gap-4">
                <div
                  ...
                  :key="video.id || index"
                  class="aspect-square rounded-lg overflow-hidden bg-[#111315] relative group cursor-pointer"
                >
                  <img
                    ...
                    class="w-full h-full object-cover"
                  />
                  <div v-else class="w-full h-full flex items-center justify-center bg-[#1a1d21]">
              ...
              <div v-else class="grid grid-cols-3 gap-4">
                <div
                  ...
                  :key="n"
                  class="aspect-square rounded-lg bg-[#111315] border-2 border-dashed border-white/[0.06] flex flex-col items-center justify-center"
                >
        ...
        <div v-else-if="activeTab === 'albums' && profileData" class="p-6">
          <div class="bg-[#111315] rounded-xl p-6 border border-white/[0.06]">
            ...
            <div
              ...
              class="bg-[#111315] rounded-lg overflow-hidden border border-white/[0.06] cursor-pointer hover:bg-[#1a1d21] transition-colors"
            >
              <div class="aspect-square bg-[#1a1d21] relative">
                <img
          ...
          <div class="bg-[#111315] rounded-xl p-6 border border-white/[0.06]">
            ...
            <div v-else-if="allValidations.length > 0" class="space-y-4">
              <div
                ...
                class="bg-[#111315] rounded-lg p-4 border border-white/[0.06] cursor-pointer hover:bg-[#1a1d21] transition-colors"
              >
        ...
        <div v-else-if="activeTab === 'groepen' && profileData" class="p-6">
          <div class="bg-[#111315] rounded-xl p-6 border border-white/[0.06]">
            ...
            >
              ...
              >
              <div class="flex items-start gap-4">
                <div class="w-16 h-16 rounded-lg bg-[#1a1d21] flex items-center justify-center shrink-0 overflow-hidden">
                  <img
        ...
        <div v-else-if="activeTab === 'speeddate' && profileData" class="p-6">
          <div class="bg-[#111315] rounded-xl p-6 border border-white/[0.06]">
            ...
            <div v-if="profileData.speeddating_active && profileData.speeddating_details" class="bg-[#1a1d21] rounded-lg p-6 border border-white/[0.06]">
        ...
        <div v-else-if="activeTab === 'parties' && profileData" class="p-6">
          <div class="bg-[#111315] rounded-xl p-6 border border-white/[0.06]">
            ...
            <a
              ...
              rel="noopener noreferrer"
              @click.stop
              class="block bg-[#111315] rounded-lg overflow-hidden border border-white/[0.06] cursor-pointer hover:bg-[#1a1d21] transition-colors"
            >
              <div v-if="party.splash_photo" class="aspect-video bg-[#1a1d21]">
                <img
        ...
        <div v-else-if="activeTab === 'following' && profileData" class="p-6">
          <div class="bg-[#111315] rounded-xl p-6 border border-white/[0.06]">
            ...
            <div
              ...
              class="bg-[#111315] rounded-lg p-4 border border-white/[0.06] flex items-center gap-4 cursor-pointer hover:bg-[#1a1d21] transition-colors"
            >
              <img
          ...
          <div class="bg-[#111315] rounded-xl p-6 border border-white/[0.06]">
            ...
            <div v-if="profileData.friends && profileData.friends.length > 0" class="space-y-3">
              <div
                ...
                class="bg-[#1a1d21] rounded-lg p-4 border border-white/[0.06] flex items-center gap-4 cursor-pointer hover:bg-white/[0.06] transition-colors"
              >
                <img
